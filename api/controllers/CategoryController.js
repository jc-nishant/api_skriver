...
module.exports = {
  ...
  addCategory: async (req, res) => {
    try {
      ...
      let existed = await Category.findOne({
        name: data.name,
        isDeleted: false,
      });
      if (existed) {
  ...
  listCategory: async (req, res) => {
    try {
      ...
      console.log(query);
      const allCategories = await Category.find(query).sort(sortBy).limit(count).skip((page - 1) * count);
      if(allCategories){
        return res.status(200).json({
            success: true,
            message: constantObj.CATEGORY.ALL_DATA,
            data: allCategories
        })
      } else {
        return res.status(400).json({
            success: false,
            message: constantObj.CATEGORY.FAILED
        })
    }
  ...
  categoryDetail: async (req, res) => {
    try {
      ...
      if (!id) {
        ...
        const categoryDetail = await Category.findOne({ id: id })
          .populate("addedBy")
          .populate("updatedBy");
