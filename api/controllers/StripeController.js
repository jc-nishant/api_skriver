...
module.exports = {
  ...
  getAllCards: async (req, res) => {
    try {
      ...
      query.userId = req.identity.id;
      let total = await Cards.count(query);
      let findusers = await Cards.find(query)
        .sort(sortBy)
        .skip(skipNo)
        .limit(count);
      return res.status(200).json({
        ...
        async (err, confirmation) => {
          if (err) {
            ...
            if (card) {
              ...
              const removedCard = await Cards.destroy({
                userId: id,
                card_id: card_id,
              });
              return res.status(200).json({
    ...
    try {
      ...
      if (get_card) {
        ...
        if (update_default_source) {
          let update_other_cards = await Cards.update(
            { userId: req.identity.id },
            { isDefault: false, updatedBy: req.identity.id }
          ).fetch();
          if (update_other_cards && update_other_cards.length > 0) {
            const set_primary_card = await Cards.updateOne(
              { userId: req.identity.id, card_id: get_card.card_id },
              { isDefault: true }
            );
            if (set_primary_card) {
              return res.status(200).json({
